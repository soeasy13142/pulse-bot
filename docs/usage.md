...
> 架构与卡片格式分别见 [[architecture.md]] / [[card-format.md]]。
...
卡片已经写入 vault（v0.1 在 VPS 端 → 5 分钟内 Windows 端可见，见 [[setup-windows.md]] 配置自动同步；取决于 [[runbook.md#f2-push-失败-卡片卡在本地]]）。
...
打开 `91_System/Dashboards/Pulse-Dashboard.md`（首次使用需先从仓库 `templates/dashboards/Pulse-Dashboard.md` 复制到 vault 对应路径），看到刚创建的卡片：
...
> **关于 `/recent` 边界**：
> - `0`、`负数`、`> 20`、`非数字`、**多余参数** → 都返回 `Usage: /recent [N] where 1 <= N <= 20`，不泄漏任何 card 内容
> - `N > 当前跟踪的卡片数` → 返回所有跟踪的卡片（不补占位符）
> - **`/promote`**：v0.2 feature，本版本未实现，不会被识别（plain text 模式接管，意图为 `reference`）
...
- **`/promote` 未实现**：手动升级 Pulse Card → 正式笔记（详见 `docs/usage.md` 后文"处理 Pulse Card 的工作流"，或上 GitHub 仓库 release notes）
- **Windows 自动同步**：v0.1 主力同步客户端已从 Mac 迁移到 Windows。配置见 [[setup-windows.md]]。
- **Mac 用户**：本仓库不再提供 Mac 端自动同步脚本（原 Mac launchd 因 macOS sandbox 限制 deferred），请手动 `git pull`
...
**Q：bot 不回消息？**
A：见 [[runbook.md#f1-bot-不响应-telegram-消息]]。

**Q：能不能发图片？**
A：v0.1 不支持。图片类想法先在 Telegram 保存或描述，等后续版本。

**Q：bot 收消息但 vault 没出现？**
A：见 [[runbook.md#f2-push-失败-卡片卡在本地]]。
...
- 设计 spec：见原仓库 `plans/2026-07-09_22-30_pulse-system-design_nogit.md`（项目历史归档）
- 实施计划：见原仓库 `plans/2026-07-09_22-45_pulse-system-implementation-plan_nogit.md`
