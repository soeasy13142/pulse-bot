...
卡片已经写入 vault（v0.1 在 VPS 端 → 5 分钟内 Mac 端可见，取决于 [[runbook.md#f2-push-失败-卡片卡在本地]]）。
...
- **Mac launchd 自动 pull deferred**：手动 `git pull --rebase --autostash`（详见 [[runbook.md#mac-端自动同步]] 维护操作节；Windows 用户见 [[setup-windows.md]] 用 `pulse-pull.ps1`）
